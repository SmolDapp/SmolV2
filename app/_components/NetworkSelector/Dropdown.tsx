...
import {IconChevronBottom} from '@lib/components/icons/IconChevronBottom';
...
function NetworkDropdownTrigger(
	props: {
		selectedNetworks: Record<number, boolean>;
		value: number[];
...
export function NetworkDropdownSelector(
	 ...
	 *************************************************************************/
	const [selectedNetworks, setSelectedNetworks] = useState<Record<number, boolean>>(
		props.value.reduce((acc: Record<number, boolean>, networkId) => {
			acc[networkId] = true;
			return acc;
		}, {})
	);
		...
		<DropdownMenu.Root onOpenChange={onOpenChange}>
			...
			<DropdownMenuContent>
				...
				<DropdownMenuCheckboxItem
					...
					onCheckedChange={() => {
						setSelectedNetworks(
							supportedNetworks.reduce((acc: Record<number, boolean>, network) => {
				...
				{supportedNetworks.map(network => (
					<DropdownMenuCheckboxItem
						...
						onCheckedChange={checked => setSelectedNetworks(prev => ({...prev, [network.id]: checked}))}>
						{network.name}
					</DropdownMenuCheckboxItem>
				))}
			</DropdownMenuContent>
