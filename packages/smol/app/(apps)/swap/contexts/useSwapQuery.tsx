import {toNormalizedBN, zeroNormalizedBN} from '@lib/utils/numbers';
...
import {isEthAddress, isZeroAddress, toAddress, truncateHex} from 'lib/utils/tools.addresses';
...
export const SwapQueryManagement = (props: {
	...
	const [hasFinishedInitialFetch, setHasFinishedInitialFetch] = useState({
		input: false,
		output: false,
		receiver: false
	});
	...
	const populateInputArgs = useCallback(async () => {
			...
			if (isEthAddress(tokenFrom)) {
				...
				newToken.token = {
					...
					balance: isZeroAddress(address)
						? zeroNormalizedBN
						: toNormalizedBN(balance.value, nativeCoin?.decimals || 0)
				};
		 ...
		 *****************************************************************************************/
		setHasFinishedInitialFetch(prev => ({...prev, input: true}));
	}, [address, configuration.input.token?.address, dispatchConfiguration, stateFromUrl, config]);
	...
	const populateOuputArgs = useCallback(async () => {
		...
		if (chainTo && tokenTo && toAddress(configuration.output.token?.address) !== toAddress(tokenTo)) {
			if (isEthAddress(tokenTo)) {
				const balance = await getBalance(config, {address: toAddress(address), chainId: chainTo});
		 ...
		 ** being called again, stating that the initial fetch has been done.
		 *****************************************************************************************/
		setHasFinishedInitialFetch(prev => ({...prev, output: true}));
	}, [address, configuration.output.token?.address, dispatchConfiguration, stateFromUrl, config]);
	...
	const populateReceiverArgs = useCallback(async () => {
		 ...
		 ** being called again, stating that the initial fetch has been done.
		 *****************************************************************************************/
		setHasFinishedInitialFetch(prev => ({...prev, receiver: true}));
