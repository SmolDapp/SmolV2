...
export const WithPrices = (props: {children: ReactElement; supportedNetworks?: Chain[]}): ReactElement => {
	const [pricesFromList, setPricesFromList] = useState<TPrices>({});
	const [fetchingQueue, setFetchingQueue] = useState<TPriceTokens>([]);
	...
	const tokensToUse = useMemo((): TPriceTokens => {
		...
		for (const token of fetchingQueue) {
			if (!_tokensToUse.find(t => t.address === token.address)) {
				_tokensToUse.push(token as TERC20TokensWithBalance);
			}
		}
		...
		async (_tokensToUse: TPriceTokens, dispatcher: Dispatch<SetStateAction<TPrices>>) => {
			...
			for (const chainID in assignedPricesFromLlama) {
				for (const address in assignedPricesFromLlama[chainID]) {
					if (!assignedPricesFromLlama[chainID][toAddress(address)]) {
						missingTokens.push({chainID: Number(chainID), address: toAddress(address)});
					} else if (assignedPricesFromLlama[chainID][toAddress(address)].raw === 0n) {
						missingTokens.push({chainID: Number(chainID), address: toAddress(address)});
					}
				}
			}
	...
	useDeepCompareEffect(() => {
		fetchLists(tokensToUse, setPricesFromList);
	}, [tokensToUse]);
	...
	const getPrices = useCallback(
		(tokens: TERC20TokensWithBalance[]): Record<number, Record<string, TNormalizedBN>> => {
			const missingPrices: Record<number, Record<string, TNormalizedBN>> = {};
			const allPrices: Record<number, Record<string, TNormalizedBN>> = {};
			for (const token of tokens) {
