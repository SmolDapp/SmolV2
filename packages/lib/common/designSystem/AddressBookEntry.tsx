...
import {Avatar} from 'lib/common/designSystem/Avatar';
...
export function AddressBookEntry(props: {
	...
	onUpdate?: (entry: TAddressBookEntry) => void;
	...
	useEffect((): void => {
		if ((ensName && !props.entry.ens) || (ensName && props.entry.ens !== ensName)) {
			props.onUpdate?.({...props.entry, ens: ensName});
		}
	}, [ensName, props, props.entry, props.onUpdate]);
				...
				<div className={'absolute inset-y-0 right-0 flex items-center'}>
					<EntryBookEntryFavorite
						...
						onClick={event => {
							...
							props.onUpdate?.({...props.entry, isFavorite: !props.entry.isFavorite});
						}}
					/>
				</div>
			</div>
